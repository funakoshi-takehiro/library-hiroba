...
- **既定はライト、ダークは PyHiroba の切替にだけ追従** — OS の配色設定は見ません（ライト表示のページの中で部品だけ黒くなる食い違いを防ぐため）。配色は両テーマで WCAG 4.5:1 以上を検証済み
...
**既定は必ずライト**です。ダークになるのは、祖先要素に `data-theme="dark"` が付いているとき（＝ PyHiroba でダークモードに切り替えたとき）だけです。

OS の `prefers-color-scheme` は意図的に参照していません。これは PyHiroba 本体と同じ方針です（`js/theme.js` — 初期状態は常にライトで、利用者が切り替えたときだけ `<html>` に属性が付く）。OS 設定を見てしまうと、**Colab がライト表示なのに OS がダークというだけで部品だけが黒くなる**、という食い違いが起きます。

そのため、背景を持つ部品（カード・アラート・表など）はすべて**不透明な色**で塗っています。ページの下地がどんな色でも、文字と背景の組み合わせが崩れません。逆に、背景を持たない部分（進捗バーのラベルや表のキャプション）は**ホストの文字色を継承**するので、暗いページでもそのまま読めます。
