...
> **対応済みです（2026-08-09、本体の `df6d049`）。** この文書は設計の記録として残しています。以下は提案時点の記述で、本体側は下記の第6節どおりに実装され、AI とのチャット（`async def` の handler）まで実機で確認されています。決めていただきたかった4点も、すべて提案どおりに決まりました。
>
> | 項目 | 決まったこと |
> |---|---|
> | 引く口 | `library_hiroba.ui.get_form(form_id)` をそのまま使用 |
> | 差し替え先 | `data-hui-output` の中だけ |
> | 登録の破棄 | 本体からは捨てず、library-hiroba の 64 件上限に任せる（本体は DOM のイベント解除のみ） |
> | `await` と非同期反復子 | ワーカー側の Python で処理（`isasyncgen` なら回しながら毎回送信、`isawaitable` なら await） |
>
> 本体がどの機能に対応しているかは、[`PYHIROBA_INTEGRATION.md`](PYHIROBA_INTEGRATION.md) の `pyhirobaFeatures` で名乗っていただく形になりました。library-hiroba はそれを見て、古い本体でだけ注意書きを出します。
